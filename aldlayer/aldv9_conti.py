...
# 不用 two-step的方式做 mix-scale v9  2*3 或者 3*2 的 中心区域
# 同时使用特征提取的方式确定中心区域
# 中心区域的形状是连续的
# 并且只在中心区域上学习打乱
class ald_fev9_onlycentre_conti(nn.Module):

    def __init__(self, num_train,central_area_size):
        super(ald_fev9_onlycentre_conti, self).__init__()
