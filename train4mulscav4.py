...
def train(nb_epoch, batch_size, exp_dir, resume=False, start_epoch=0, model_path=None, ald_lr=0.002 , base_lr=0.002,use_mgpu=False):
    # setup output
    exp_dir = exp_dir
    try:
        os.stat(exp_dir)
    ...
    net = model4mulscalev4( num_train)
    net = net.cuda()
    if use_mgpu:
    ...
    result_4_save = []
    # result_8_save = []
    resultmul_centre_save = []
    resultmul_boundary1_save = []
    resultmul_boundary2_save = []
    ...
    for epoch in range(start_epoch, nb_epoch):
        ...
        for batch_idx, data in enumerate(trainloader):
            ...
            # Step 1
            optimizer.zero_grad()
            output_1, _, _, _, resultmul_boundary1,resultmul_boundary2  ,resultmul_centre = net(inputs, 'mul', batch_idx, batch_size, tra)
            # print(3)
            ...
            # 保存每次变化矩阵的结果
            if (epoch <= 29):
                resultmul_central_cpu = resultmul_centre[:, 0, :, :].detach().cpu()
                resultmul_centre_save.append(resultmul_central_cpu)
            if (epoch <= 29):
                resultmul_boundary1_cpu = resultmul_boundary1[:, 0, :, :].detach().cpu()
                resultmul_boundary1_save.append(resultmul_boundary1_cpu)
                resultmul_boundary2_cpu = resultmul_boundary2[:, 0, :, :].detach().cpu()
                resultmul_boundary2_save.append(resultmul_boundary2_cpu)
            # # 输入原图
            # optimizer.zero_grad()
            # # inputs1 = jigsaw_generator(inputs, 8)
            # output_1, _, _, _,result_8 = net(inputs, '8', batch_idx, batch_size, tra)
            # # print(3)
            # loss1 = CELoss(output_1, targets) * 1
            # loss1.backward()
            # optimizer.step()
            # # 保存每次变化矩阵的结果
            # if (epoch <= 29):
            #     result_8_cpu = result_8[:, 0, :, :].detach().cpu()
            #     result_8_save.append(result_8_cpu)
            ...
            # Step 4
            optimizer.zero_grad()
            _, _, _, output_concat = net(inputs, '1', batch_idx, batch_size, tra)
        ...
        if epoch == 29:
            ...
            resultmul_centre_npy = np.vstack(resultmul_centre_save)
            resultmul_boundary1_npy = np.vstack(resultmul_boundary1_save)
            resultmul_boundary2_npy = np.vstack(resultmul_boundary2_save)
            print(result_2_npy.shape)
            print(result_4_npy.shape)
            print(resultmul_boundary1_npy.shape)
            print(resultmul_boundary2_npy.shape)
            print(resultmul_centre_npy.shape)
            resultmul_centre = './' + exp_dir + '/' + 'resultmul_centre.npy'
            resultmul_boundary1 = './' + exp_dir + '/' + 'resultmul_boundary1.npy'
            resultmul_boundary2 = './' + exp_dir + '/' + 'resultmul_boundary2.npy'
            ...
            np.save(resultmul_centre, resultmul_centre_npy)
            np.save(resultmul_boundary1, resultmul_boundary1_npy)
            np.save(resultmul_boundary2, resultmul_boundary2_npy)
        ...
        val_acc, val_acc_com, val_loss = test_mulv4(net, CELoss, 32)
