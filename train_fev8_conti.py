...
def parse_args():
    ...
    parser.add_argument('--central_area_size', dest='central_area_size',
...
def train(nb_epoch, batch_size, exp_dir, resume=False, start_epoch=0, model_path=None, ald_lr=0.002 , base_lr=0.002,
     use_mgpu=False,dset='cub',back_bone='resnet50_pmg',central_area_size='2*4'):
    # setup output
    exp_dir = exp_dir
    try:
        os.stat(exp_dir)
    ...
    net = model4fev8_onlycentral_conti( num_train,dset,back_bone,central_area_size)
    net = net.cuda()
    if use_mgpu :
...
if __name__ == '__main__':
    ...
    central_area_size = args.central_area_size
    train(nb_epoch=epoch,  # number of epoch
          ...
          dset = dset,
          back_bone = back_bone,
          central_area_size = central_area_size
          )
