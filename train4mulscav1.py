...
def train(nb_epoch, batch_size, exp_dir, resume=False, start_epoch=0, model_path=None, ald_lr=0.002 , base_lr=0.002,use_mgpu=False,dset='cub'):
    # setup output
    exp_dir = exp_dir
    try:
        os.stat(exp_dir)
    ...
    net = model4mulscalev1( num_train,dset=dset)
    net = net.cuda()
    if use_mgpu:
    ...
    for epoch in range(start_epoch, nb_epoch):
        ...
        for batch_idx, data in enumerate(trainloader):
            ...
            # Step 1
            optimizer.zero_grad()
            output_1, _, _, _, resultmul_centre  ,resultmul_global = net(inputs, 'mul', batch_idx, batch_size, tra)
            # print(3)
            ...
            if (epoch <= 29):
                ...
                resultmul_global_cpu = resultmul_global[:, 0, :, :].detach().cpu()
                resultmul_global_save.append(resultmul_global_cpu)
            # # 输入原图
            # optimizer.zero_grad()
        ...
        if epoch == 29:
            ...
            resultmul_global_npy = np.vstack(resultmul_global_save)
            print(result_2_npy.shape)
            print(result_4_npy.shape)
            print(resultmul_global_npy.shape)
            print(resultmul_centre_npy.shape)
            resultmul_centre = './' + exp_dir + '/' + 'resultmul_centre.npy'
            resultmul_global = './' + exp_dir + '/' + 'resultmul_global.npy'
            ...
            np.save(resultmul_global, resultmul_global_npy)
        ...
        val_acc, val_acc_com, val_loss = test_mulv1(net, CELoss, 32,dset=dset)
